...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (signInResult?.error) {
      setError("تم التسجيل بنجاح، ولكن فشل تسجيل الدخول التلقائي. يرجى تسجيل الدخول يدوياً.");
      setLoading(false);
    } else {
      router.push("/");
      router.refresh();
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4 relative overflow-hidden py-12 text-right" dir="rtl">
      {/* Background Orbs */}
      ...
      <div className="w-full max-w-lg relative">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 mt-2">
            انضم إلى باور ستور لإدارة طلباتك بسهولة
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="relative group">
                    <User className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-500 group-focus-within:text-amber-500 transition-colors" />
                    <input
                    type="text"
                    name="firstName"
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-300 mr-1">
                رقم الهاتف <span className="text-gray-600 font-normal">(اختياري)</span>
              </label>
              <div className="relative group">
                <Phone className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-500 group-focus-within:text-amber-500 transition-colors" />
                <input
                  type="tel"
                  name="phone"
                  className="w-full bg-gray-950/50 border border-white/10 rounded-xl py-3 pr-12 pl-4 text-white placeholder:text-gray-600 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 transition-all text-right"
        ...
        <div className="mt-8 text-center">
            <Link href="/" className="text-gray-600 hover:text-gray-400 transition-colors inline-flex items-center gap-2 text-sm text-right">
