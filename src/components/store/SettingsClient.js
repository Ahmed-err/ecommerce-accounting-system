...
export default function SettingsClient({ user }) {
  ...
  const inputClass = (extra = "") =>
    `w-full bg-muted border-2 border-transparent rounded-2xl px-6 py-4 text-foreground placeholder:text-muted-foreground focus:border-amber-500 transition-all outline-none font-semibold ${isRTL ? "text-right" : "text-left"} ${extra}`;

  return (
    <div className={`space-y-12 ${isRTL ? "text-right" : "text-left"}`} dir={isRTL ? "rtl" : "ltr"}>
      {/* Profile Hub Header */}
      <div className="relative overflow-hidden bg-card border border-border rounded-[40px] p-8 md:p-12 mb-8 group hover:bg-muted/20 transition-all duration-500">
        ...
        <div className="flex flex-col md:flex-row items-center gap-8 relative z-10">
          ...
          <div className="flex-1 text-center md:text-start">
            ...
            <p className="text-muted-foreground text-lg mb-6 max-w-lg">{user.email}</p>
            <div className={`flex flex-wrap gap-2 ${isRTL ? "justify-end" : "justify-start"}`}>
              <span className="px-4 py-1.5 bg-muted border border-border rounded-full text-xs text-muted-foreground font-medium">
                Sudan, Khartoum
              </span>
              <span className="px-4 py-1.5 bg-muted border border-border rounded-full text-xs text-muted-foreground font-medium regular-nums">
      ...
      {message.text && (
        <div
          className={`p-6 rounded-[24px] flex items-center gap-4 animate-in fade-in slide-in-from-top-4 duration-300 ${
            message.type === "success"
              ? "bg-emerald-500/10 border border-emerald-500/30 text-emerald-700 dark:text-emerald-400"
              : "bg-red-500/10 border border-red-500/30 text-red-700 dark:text-red-400"
          }`}
        >
          {message.type === "success" ? <CheckCircle className="h-6 w-6 shrink-0" /> : <AlertCircle className="h-6 w-6 shrink-0" />}
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-card border border-border rounded-[40px] p-8 md:p-12 space-y-10 group/card hover:bg-muted/10 transition-all duration-500">
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground text-sm ml-9">
              {lang === "ar" ? "تحديث معلوماتك الشخصية وصورة الحساب." : "Update your personal information and profile picture."}
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-3">
              <label className="text-sm font-bold text-muted-foreground uppercase tracking-widest px-1">{t.employeesFirstName}</label>
              <input type="text" name="firstName" value={formData.firstName} onChange={handleChange} required className={inputClass()} />
            </div>
            <div className="space-y-3">
              <label className="text-sm font-bold text-muted-foreground uppercase tracking-widest px-1">{t.employeesLastName}</label>
            ...
            <div className="space-y-3">
              ...
              <input type="tel" name="phone" value={formData.phone} onChange={handleChange} placeholder="+249..." className={inputClass("regular-nums")} />
            </div>
            <div className="space-y-3">
              ...
              <input
                ...
                value={user.email}
                disabled
                className={`w-full bg-muted/50 border-2 border-border rounded-2xl px-6 py-4 text-muted-foreground cursor-not-allowed outline-none ${isRTL ? "text-right" : "text-left"} font-medium`}
              />
            </div>
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-[40px] p-8 md:p-12 space-y-10 group/sec hover:bg-muted/10 transition-all duration-500">
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground text-sm ml-9">
              {lang === "ar" ? "تأمين حسابك بكلمة مرور قوية." : "Secure your account with a strong password."}
            </p>
          </div>

          <div className="space-y-8">
            <div className="space-y-3 max-w-md">
              <label className="text-sm font-bold text-muted-foreground uppercase tracking-widest px-1">{t.settingsCurrentPassword}</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4 border-t border-border">
              <div className="space-y-3">
                <label className="text-sm font-bold text-muted-foreground uppercase tracking-widest px-1">{t.settingsNewPassword}</label>
                <div className="relative">
                  <input
              ...
              <div className="space-y-3">
                <label className="text-sm font-bold text-muted-foreground uppercase tracking-widest px-1">{t.settingsConfirmPassword}</label>
                <div className="relative">
                  <input
